...
export function StandardHours() {
  return (
    <section className={styles.section} id="standard-hours">
      ...
      <p>Fridays and Saturdays are the undisputed financial lifeblood of the casual dining industry. To maximize revenue from late-night movies, sporting events, and the younger bar-hopping demographic, Applebee's significantly extends its operating hours. The standard closing time on a Friday or Saturday night is <strong>1:00 AM</strong>. In heavily populated urban centers or college towns, this is frequently extended to <strong>2:00 AM</strong>. It&apos;s incredibly rare to find an Applebee's open past 2:00 AM due to state-mandated liquor sale curfews.</p>
    </section>
  );
}
...
export function LateNightShift() {
  return (
    <section className={styles.section} id="late-night-happy-hour">
      ...
      <p>When navigating late-night hours, diners must understand the concept of "Last Call." If an Applebee's closes at 1:00 AM, you can&apos;t walk in at 12:55 AM and expect a three-course meal. The kitchen typically begins shutting down and cleaning the fryers 30 minutes prior to the posted closing time. The bartender will announce "Last Call" for alcohol approximately 20 to 30 minutes before closing to ensure all drinks are consumed before local law mandates the removal of glassware from the tables.</p>
    </section>
  );
}
...
export function TakeoutCurfews() {
  return (
    <section className={styles.section} id="takeout-curfews">
      ...
      <p>One of the most frustrating modern dining experiences is driving to an Applebee's that&apos;s clearly open, only to find that the DoorDash app claims the restaurant is "Closed." This isn&apos;t a glitch; it&apos;s a meticulously engineered digital curfew designed to protect the kitchen staff during closing procedures.</p>
      ...
      <p>Why? Because a delivery driver often takes 15 to 20 minutes to arrive at the restaurant. If a massive order of Fiesta Lime Chicken is accepted at 11:58 PM, the kitchen staff can&apos;t break down the cooking line, the driver won&apos;t arrive until 12:15 AM, and the franchise incurs massive overtime labor costs. If you want late-night delivery, you must submit your digital order at least 45 minutes before the posted physical closing time.</p>
    </section>
  );
}
...
export function VerifyRealTime() {
  return (
    <section className={styles.section} id="verify-hours">
      ...
      <p>The cardinal rule of casual dining in 2026 is simple: Never blindly trust a third-party search engine on a major holiday. Google Maps relies on algorithmic data scraping and user-submitted edits. If a franchise owner decides at 3:00 PM on Thanksgiving Day to close the restaurant early because it&apos;s dead, Google Maps will still insist the location is open until Midnight.</p>
