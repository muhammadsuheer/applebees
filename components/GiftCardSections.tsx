...
export function GiftCardOverview() {
  return (
    <section className={styles.section} id="gift-card-ecosystem">
      ...
      <p>The Applebee's gift card is one of the most widely circulated restaurant currencies in the United States. In 2026, it operates as a dual-tier financial system, consisting of both traditional physical plastic cards and instantly deliverable digital eGift cards. Whether you&apos;re purchasing a $25 card as a last-minute birthday present or placing a $50,000 corporate order for an employee incentive program, understanding the strict rules governing these cards is essential.</p>
...
export function BonusCardTrap() {
  return (
    <section className={styles.section} id="bonus-card-trap">
      ...
      <p>Every holiday season (typically spanning from late November through December), Applebee's runs a massive, nationally televised promotion: <em>Buy a $50 Gift Card, Get a $10 Bonus Card</em>. This promotion generates tens of millions of dollars in revenue, but it&apos;s also the source of immense consumer frustration at the host stand.</p>
      
      <h3>The Legal Distinction</h3>
      <p>You must understand the strict legal difference between a "Gift Card" and a "Bonus Card." Under the federal Credit CARD Act of 2009, a purchased gift card (the $50 card you actually paid for) can&apos;t expire for at least five years, and Applebee's corporate policy dictates that they <strong>never expire</strong> and are never subject to dormancy fees.</p>
      
      <p>However, the $10 "Bonus Card" is legally classified as a promotional coupon, not cash. Because you didn&apos;t purchase it, federal expiration laws don&apos;t apply. These Bonus Cards have incredibly strict expiration dates, often expiring just six to eight weeks after the holiday promotion ends (typically around late February or early March).</p>
      
      <h3>The Host Stand Confrontation</h3>
      <p>If you attempt to use a $10 Bonus Card in April, the POS system will aggressively reject it. The manager can&apos;t override this rejection because the promotional code has been purged from the corporate servers. To avoid this trap, you must immediately spend your Bonus Cards in January and save the actual Gift Cards for later in the year.</p>
    </section>
  );
}
...
export function CorporateBulk() {
  return (
    <section className={styles.section} id="corporate-bulk">
      ...
      <p>Applebee's operates a massive B2B (Business-to-Business) gift card division designed to service Fortune 500 companies, massive school districts, and national charities. If you&apos;re an office manager looking to buy 500 gift cards for a company holiday party, you should never buy them at retail price.</p>
...
export function ThirdPartyRetailers() {
  return (
    <section className={styles.section} id="third-party">
      ...
      <p>You don&apos;t need to walk into a physical Applebee's to purchase a gift card. In fact, purchasing them outside of the restaurant is often a vastly superior financial strategy.</p>
      
      <h3>The Wholesale Club Arbitrage (Costco & Sam's Club)</h3>
      <p>The absolute best place to buy an Applebee's gift card is a wholesale warehouse club. Costco and Sam's Club routinely sell multi-packs of Applebee's gift cards at an aggressive 20% discount. For example, you can frequently purchase a bundle of four $25 gift cards ($100 total value) for exactly $79.99. If you&apos;re a frequent Applebee's diner, failing to utilize this wholesale arbitrage is equivalent to throwing a 20% tip in the trash.</p>
...
export function LostStolen() {
  return (
    <section className={styles.section} id="lost-stolen">
      ...
      <p>Applebee's corporate policy explicitly states they aren&apos;t responsible for lost or stolen cards. However, there&apos;s a narrow window for recovery if you retained the original activation receipt. The activation receipt contains the card's serial number (distinct from the 19-digit spending number).</p>
      <p>If you realize the card is stolen, you must immediately call 1-800-252-6722. If you can provide the exact serial number from the receipt, the corporate office can freeze the card. If the thief hasn&apos;t already spent the funds, Applebee's will transfer the remaining balance to a newly issued card and mail it to you.</p>
    </section>
  );
}
...
export function DigitalWallet() {
  return (
    <section className={styles.section} id="digital-wallet">
      ...
      <p>When you purchase an eGift card online, the confirmation email will contain an "Add to Wallet" button. Tapping this button injects a dynamic digital card directly into your phone. When it&apos;s time to pay your bill at the restaurant, you simply present the digital barcode on your screen to the server, who will scan it with their handheld POS tablet. This ensures you never leave a physical plastic card sitting on a sticky dining room table.</p>
    </section>
  );
}
...
export function GiftCardFAQ() {
  const faqData = [
    { question: "Do Applebee's gift cards expire?", answer: "No. Standard Applebee's gift cards never expire and aren't subject to any dormancy or maintenance fees. However, promotional 'Bonus Cards' usually expire within 6 to 8 weeks." },
