...
export function DirectVsThirdParty() {
  return (
    <section className={styles.section} id="direct-vs-third-party">
      ...
      <p>Here is the industry secret that tech companies don&apos;t want you to know: If you order "Delivery" directly through the official Applebee's App or Applebees.com, the restaurant doesn&apos;t actually employ their own fleet of delivery drivers. Instead, Applebee's utilizes a "White Label" delivery service. The order is automatically routed into the kitchen's Point of Sale (POS) system, and the corporate software quietly summons a DoorDash or Uber driver to fulfill the delivery.</p>

      <p>Why does this matter? Because when you order through the official Applebee's App, you&apos;re purchasing the food directly from the restaurant at the <strong>standard, un-inflated retail price</strong>. You will pay a transparent, flat delivery fee (often around $4.99), but you bypass the massive menu markups and exorbitant "Service Fees" charged by the third-party platforms. In 2026, ordering direct is mathematically the most cost-effective way to have Applebee's delivered to your door.</p>

      <h3>The Customer Service Black Hole</h3>
      <p>Price isn&apos;t the only differentiator. When you order through UberEats or Grubhub, your financial transaction is with the tech company, not the restaurant. If your order arrives missing the Spinach & Artichoke Dip, calling the physical Applebee's location will yield zero results. The restaurant manager can&apos;t issue a refund because they don&apos;t hold your money; you must navigate the frustrating, automated chatbot systems of the third-party app to beg for a credit. Conversely, ordering directly through Applebee's keeps the transaction in-house, allowing local management to resolve issues rapidly.</p>
    </section>
  );
}
...
export function HiddenMarkup() {
  return (
    <section className={styles.section} id="hidden-markup">
      ...
      <p>Applebee's franchise owners operate on tight margins (usually 3% to 5% net profit). They absolutely can&apos;t afford to hand over 30% of a transaction to DoorDash. Therefore, they deploy the standard industry defense mechanism: <strong>Menu Price Inflation</strong>. </p>
      <p>If you walk into a physical Applebee's, a Classic Bacon Cheeseburger might cost $12.99. If you open DoorDash while sitting in that exact same booth, that identical burger will likely be listed at $16.89. This isn&apos;t a glitch; it&apos;s a calculated markup designed to pass the tech company's commission fee directly onto the consumer. </p>
...
export function CarsideToGo() {
  return (
    <section className={styles.section} id="carside-to-go">
      ...
      <p>Every Applebee's location features dedicated "Carside To Go" parking spots, typically located directly near the front entrance. These spots are highly visible from the host stand. When you arrive, you don&apos;t need to leave your car. You simply open the Applebee's App, tap the "I'm Here" button, and input your parking spot number and vehicle description (e.g., Spot 3, Silver Honda Civic). </p>
      ...
      <p>Carside To Go is unequivocally the most cost-effective off-premise dining option. You pay the exact, un-inflated physical menu price. There are zero delivery fees. There are zero hidden service fees. While tipping the To-Go Specialist is highly recommended and customary (they&apos;re often paid a tipped minimum wage), a standard 10% to 15% tip is vastly cheaper than the 40% aggregate premium of utilizing a delivery app.</p>
    </section>
  );
}
...
export function GhostKitchens() {
  return (
    <section className={styles.section} id="ghost-kitchens">
      ...
      <p>You must be incredibly careful when interacting with these ghost kitchens. Because Cosmic Wings and Applebee's are listed as two separate entities on delivery platforms, you can&apos;t combine items from both menus into a single order. If your spouse wants an Applebee's Fiesta Lime Chicken and you want Cosmic Wings Flamin' Hot tenders, you must place two separate orders, pay two separate delivery fees, and tip two separate driverseven though both meals are being cooked by the exact same chef in the exact same building.</p>
    </section>
  );
}
...
export function AlcoholDelivery() {
  return (
    <section className={styles.section} id="alcohol-delivery">
      ...
      <p>Even where legal, ordering alcohol isn&apos;t a seamless experience:</p>
      <ul>
        <li><strong>Tamper-Proof Packaging:</strong> The restaurant is legally required to seal the cocktail in a tamper-proof container. You won&apos;t receive a standard plastic cup with a straw; it will be vacuum-sealed or taped shut to prevent the driver from consuming it en route.</li>
        <li><strong>Food Requirement:</strong> Many states stipulate that alcohol can only be delivered if it&apos;s accompanied by a substantial food purchase. You can&apos;t order *just* a Margarita.</li>
        <li><strong>The ID Scan:</strong> Delivery drivers are legally required to scan your physical ID using their app upon arrival. They can&apos;t leave the alcohol on your porch. If you aren&apos;t home, if you&apos;re underage, or if you&apos;re visibly intoxicated, the driver is legally obligated to return the alcohol to the restaurant, and you won&apos;t be refunded.</li>
      </ul>
    </section>
  );
}
...
export function PackagingScience() {
  return (
    <section className={styles.section} id="packaging-science">
      ...
      <p>You may notice that certain highly delicate items (like specific sizzling skillets or ice cream-based desserts) are occasionally absent from the delivery menu. This is a deliberate choice by corporate chefs. If a dish can&apos;t survive a 25-minute journey in a driver's insulated bag without turning into an unappetizing mess, it&apos;s removed from the delivery ecosystem entirely to protect the brand's reputation.</p>
    </section>
  );
}
...
export function CateringVsDelivery() {
  return (
    <section className={styles.section} id="catering-vs-delivery">
      ...
      <p>If you&apos;re hosting a Super Bowl party, an office luncheon, or a large family gathering, relying on standard delivery apps is a catastrophic financial error. Ordering 10 individual entrees on DoorDash subjects you to massive cumulative markups and exorbitant service fees.</p>
...
export function DeliveryFAQ() {
  const faqData = [
    ...
    { question: "Can I get Applebee's Happy Hour deals delivered?", answer: "No. The famous Half-Price Appetizers and Happy Hour drink specials are strictly 'Dine-In Only' promotions designed to drive foot traffic into the physical restaurant. They don't apply to delivery or Carside To Go." },
